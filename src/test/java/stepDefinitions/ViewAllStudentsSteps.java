...
public class ViewAllStudentsSteps {
    ...
    public void i_navigate_to_view_all_students_page() {
        ...
        test.log(Status.INFO, "Navigated to View All Students page");
    ...
    public void student_data_is_loaded_from(String csvFile) {
        ...
        test.log(Status.INFO, "Loaded " + studentData.size() + " records from " + csvFile);
    }
    ...
    public void the_user_validates_each_student_by_roll_number() {
        for (Map<String,String> student : studentData) {
            ...
            test.log(Status.INFO, String.format("Roll %s → expectedPresent=%s", rollNo, expectedExists));

            boolean actualPresent = viewPage.isStudentRowPresent(rollNo);
            test.log(Status.INFO, String.format("Roll %s → actualPresent=%s", rollNo, actualPresent));
            ...
            if (actualPresent) {
                ...
                test.info("Scrolled to row for roll number: " + rollNo);
            }
            ...
            if (expectedExists) {
                if (!actualPresent) {
                    test.fail("Expected row for roll " + rollNo + " but none found");
                    Assert.fail("Row missing for expected roll number: " + rollNo);
                }
                test.pass("Row found for roll number: " + rollNo);

                // validate details
                List<String> actual = viewPage.getStudentDetailsByRollNo(rollNo);
                test.info("Extracted details: " + actual);

                // scroll details into view
                WebElement rowElem = viewPage.getRowByRollNo(rollNo);
                ((JavascriptExecutor) driver)
                    .executeScript("arguments[0].scrollIntoView({behavior:'smooth',block:'center'});", rowElem);
                test.info("Scrolled to details for roll number: " + rollNo);
                ...
                for (int i = 0; i < expected.size(); i++) {
                    ...
                    test.pass(String.format("Verified column[%d] for roll %s: [%s]", i, rollNo, act));
                }
            }
            // expected NOT to exist
            else {
                if (actualPresent) {
                    test.fail("Expected NO row for roll " + rollNo + " but found one");
                    Assert.fail("Unexpected row found for roll number: " + rollNo);
                }
                test.pass("No row found for roll number (as expected): " + rollNo);
            }
        }
    }
    ...
    public void all_student_details_should_match_the_expected_values() {
        test.log(Status.PASS, "All student row‐existence and details validated.");
    }
