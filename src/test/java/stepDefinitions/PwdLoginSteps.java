...
public class PwdLoginSteps {
    ...
    public void i_open_the_login_page() {
        ...
        ReportHooks.test.log(Status.INFO, "Navigated to login page: " + BaseClass.getBaseUrl());
    }
    ...
    public void validate_password_of_type_using_data_from(String type, String csvPath) {
        ReportHooks.test.log(Status.INFO, "Reading test data for password type: " + type);
        ...
        login.clickSignIn();
        ReportHooks.test.log(Status.INFO, "Submitted form with password: " + (input.isEmpty() ? "<blank>" : "<provided>"));
        ...
        try {
            if ("Success".equalsIgnoreCase(expected)) {
                ...
                if (!errorDisplayed) {
                    String base64Screenshot = ScreenshotUtil.takeScreenshotAsBase64(driver, "Pass_" + type, false);
                    if (base64Screenshot != null) {
                        ReportHooks.test.pass("[" + type + "] Passed: no error displayed", 
                            com.aventstack.extentreports.MediaEntityBuilder.createScreenCaptureFromBase64String(base64Screenshot).build());
                    } else {
                        ReportHooks.test.pass("[" + type + "] Passed: no error displayed");
                    }
                } else {
                    String base64Screenshot = ScreenshotUtil.takeScreenshotAsBase64(driver, "Fail_" + type, false);
                    if (base64Screenshot != null) {
                        ReportHooks.test.fail("[" + type + "] FAILED: unexpected error shown", 
                            com.aventstack.extentreports.MediaEntityBuilder.createScreenCaptureFromBase64String(base64Screenshot).build());
                    } else {
                        ReportHooks.test.fail("[" + type + "] FAILED: unexpected error shown");
                    }
            ...
            ReportHooks.test.log(Status.FAIL, "Assertion failed: " + ae.getMessage());
            throw ae;
        } catch (Exception ex) {
            ReportHooks.test.log(Status.FAIL, "Exception during validation: " + ex.getMessage());
            throw ex;
        }
    }
    ...
    public void final_validation() {
        ReportHooks.test.log(Status.INFO, "Final validation step (all assertions handled in @When)");
    }

    // Helper for logging errors and attaching screenshots
    private void validateError(String type, String expected, String actual) {
        String label = "[" + type + "]";
        String base64Screenshot = ScreenshotUtil.takeScreenshotAsBase64(driver, "Error_" + type, false);

        if (actual.equals(expected)) {
            if (base64Screenshot != null) {
                ReportHooks.test.pass(label + " Passed: expected error '" + expected + "'", 
                    com.aventstack.extentreports.MediaEntityBuilder.createScreenCaptureFromBase64String(base64Screenshot).build());
            } else {
                ReportHooks.test.pass(label + " Passed: expected error '" + expected + "'");
            }
            ...
            if (base64Screenshot != null) {
                ReportHooks.test.fail(label + " FAILED: expected '" + expected + "' but got '" + actual + "'", 
                    com.aventstack.extentreports.MediaEntityBuilder.createScreenCaptureFromBase64String(base64Screenshot).build());
            } else {
                ReportHooks.test.fail(label + " FAILED: expected '" + expected + "' but got '" + actual + "'");
            }
